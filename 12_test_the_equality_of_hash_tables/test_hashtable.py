...
def test_should_return_duplicate_values(hash_table):
    hash_table = HashTable(capacity=100)
    hash_table["Alice"] = 24
    hash_table["Bob"] = 42
    hash_table["Joe"] = 42
...
def test_should_convert_to_dict(hash_table):
    ...
    assert set(dictionary.values()) == unordered(hash_table.values)
...
def test_should_compare_equal_different_capacity(hash_table):
    data = {"a": 1, "b": 2, "c": 3}
